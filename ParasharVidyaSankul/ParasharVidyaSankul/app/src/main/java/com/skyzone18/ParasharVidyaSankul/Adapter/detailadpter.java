...
public class detailadpter extends RecyclerView.Adapter<detailadpter.ViewHolder> {
    ...
    public void onBindViewHolder(final detailadpter.ViewHolder holder, final int position) {
        ...
        holder.webDesc.loadDataWithBaseURL(null, ((Datum) this.detailst.get(position)).getDetail(), "text/html", "utf-8", null);
        holder.tvSubject.setText(detailst.get(position).getSub());
        Glide.with(contet).load(detailst.get(position).getImagepath())
                .crossFade()
                //.thumbnail(0.5f)
